...
class Test_Flash:
    def setup_class(self):
        ...
        # 初始化信息
        self.g4_serialno = global_config.getRaw("config-G4", "g4_serialno")
        self.g4 = models.Glocalme(device_id=self.g4_serialno, log_project=log)
        self.g4.wait_device_connect()
        self.g4.wakey()  # 设置辅助机屏幕常亮
        ...
        self.g4_d = u2.connect(self.g4_serialno)

        self.version = self.g4.get_current_version()  # 获取测试设备的版本
        ...
        if self.g4_d(resourceId="com.glocalme.g4home:id/tv_unlock").exists():
            self.g4_d.swipe(0.5, 0.8, 0.5, 0.3, 0.1)
        self.g4.root_by_at()
        self.g4_d.app_start(package_name="com.glocalme.g4home", activity="com.glocalme.basic.more.MoreActivity")
...
class Test_Display:
    ...
    def test01(self):
        '''测试用例：卡状态'''
        log.info("start %s" % sys._getframe().f_code.co_name)
        log.info(self.g4_d.device_info)
        assert True
...
class Test_Fota:
    def setup_class(self):
        ...
        # 初始化信息
        self.g4_serialno = global_config.getRaw("config-G4", "g4_serialno")
        self.source_version = global_config.getRaw("config-G4", "source_version")
        self.target_version = global_config.getRaw("config-G4", "target_version")
        self.g4 = models.G4(device_id=self.g4_serialno, log_project=log)
        self.g4.wait_device_connect()
        self.g4.wakey()  # 设置屏幕常亮
        # 执行uiautomator2的init操作，在测试设备端安装uiautomator app，minicap和minitouch，atx-agent
        uiautomator2_init = subprocess.getoutput("python -m uiautomator2 init")
        self.g4_d = u2.connect(self.g4_serialno)
    ...
    def setup(self):
        current_version = self.g4.get_current_version()
        if current_version != self.source_version:
            self.g4.root_by_at()
            self.g4.local_update(upgradePackage_file="E:\\2-version\\G4\\FOTA_G4S18_TSV1.4.001.016.210204_143828_user.zip")
            self.g4.wait_device_disconnect()
            self.g4.wait_device_connect(timeout=480)
            self.g4.wait_network_connect()
            # time.sleep(30)
            # 执行uiautomator2的init操作，在测试设备端安装uiautomator app，minicap和minitouch，atx-agent
            # uiautomator2_init = subprocess.getoutput("python -m uiautomator2 init")
    ...
    def test01(self):
        ...
        log.info("start %s" % sys._getframe().f_code.co_name)
        self.g4.root_by_at()
        self.g4.start_activity(package_name='com.glocalme.g4home', activity='com.glocalme.basic.simcard.SimCardActivity')
        if not self.g4_d(resourceId="com.glocalme.g4home:id/iv_vsim").exists():
            # 选择云卡登陆
            self.g4_d(resourceId="com.glocalme.g4home:id/rl_vsim").click()
            self.g4_d(resourceId="com.glocalme.g4home:id/tb_confirm").click()
            time.sleep(5)
        self.g4.reboot()
        self.g4.wait_device_connect()
        self.g4.wakey()
        self.g4.wait_network_connect()
        check_result = pytest.assume(self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_new_version").exists(timeout=180))
        if check_result:
            self.ws1["F2"] = "PASS"
        else:
            self.ws1["F2"] = "FAIL"
    ...
    def test02(self):
        ...
        iportLog_file = self.g4.pull_iportLog(target_path=self.test_record_path)
        next_check_time = self.g4.next_check_time(iportLog_file=iportLog_file)
        # 将下次周期检测的时间转换为时间戳
        ...
        self.g4.set_system_time(timestamp=set_system_timestamp)
        time.sleep(120)
        circle_check_result = pytest.assume(self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_new_version").exists(timeout=180))
    ...
    def test03(self):
        ...
        if not self.g4_d(resourceId="com.glocalme.g4home:id/iv_vsim").exists():
            ...
            time.sleep(5)
        self.g4.wait_network_connect()
        self.g4.start_activity(package_name="com.abupdate.fota_demo_iot", activity="com.abupdate.fota_demo_iot.view.activity.MainActivity")
        if self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_update_state_detail", text="下载").exists(30):
            self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_update_state_detail", text="下载").click()
        download_result = pytest.assume(self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_update_state_detail", text="点击升级").exists(timeout=360))
        if download_result:
            self.ws1["F4"] = "PASS"
        else:
            self.ws1["F4"] = "FAIL"
        # if self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_update_state_detail").exists():
        #     self.g4.wait_device_disconnect()

    def test04(self):
        """测试用例：云卡网络-手动升级"""
        log.info("start %s" % sys._getframe().f_code.co_name)
        if self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_update_state_detail", text="点击升级").exists():
            self.g4_d(resourceId="com.abupdate.fota_demo_iot:id/tv_update_state_detail", text="点击升级").click()
            self.g4.wait_device_disconnect()
            self.g4.wait_device_connect(timeout=480)
            self.g4.wait_network_connect()
            upgraded_version = self.g4.get_current_version()
    ...
    def test05(self):
        ...
        self.g4.root_by_at()
        self.g4.start_activity(package_name='com.glocalme.g4home', activity='com.glocalme.basic.simcard.SimCardActivity')
        if not self.g4_d(resourceId="com.glocalme.g4home:id/iv_sim1").exists():
            # 选择实体卡登陆
            self.g4_d(resourceId="com.glocalme.g4home:id/rl_sim1").click()
            self.g4_d(resourceId="com.glocalme.g4home:id/tb_confirm").click()
            time.sleep(5)
        self.g4.reboot()
        self.g4.wait_device_connect()
