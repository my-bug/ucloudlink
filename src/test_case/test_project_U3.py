...
log = models.Logger(screen_output=True, log_level='logging.INFO', log_file=log_file).create_logger()
...
class Test_CloudSimAndPhysicalSimSwitch:
    ...
    def setup_class(self):
        ...
        self.test_record_path = '/'.join([BASEDIR, 'TestRecord', project_name, "Test_CloudSimAndPhysicalSimSwitch"])
        if not os.path.exists(self.test_record_path):
            os.makedirs(self.test_record_path)

        # 初始化信息
        self.u3_serialno = global_config.getRaw("config-U3", "u3_serialno")
        self.s20i_serialno = global_config.getRaw("config-S20i", "s20i_serialno")
        self.u3_ssid = global_config.getRaw("config-U3", "u3_ssid")
        self.u3_password = global_config.getRaw("config-U3", "u3_password")
        self.u3 = models.U3(device_id=self.u3_serialno, log_project=log)
        self.s20i = models.Glocalme(device_id=self.s20i_serialno, log_project=log)
        ...
        self.s20i_d = u2.connect(self.s20i_serialno)
        self.s20i_uiOpration = S20i_uiOpration(d=self.s20i_d, log_project=log)
        ...
        # 使用S20i手机查找并连接U3设备的wifi
        self.s20i_uiOpration.connect_wifi(wifi_name=self.u3_ssid, password=self.u3_password)
        self.s20i_uiOpration.open_webui()
        if self.s20i_d(resourceId="status_info").exists():
            # 获取portal界面的显示语言，如果语言不是English，就将语言设置为English
            portal_language = self.s20i_d(resourceId="status_info").child()[1].get_text()
            if portal_language != "English":
                self.s20i_d(resourceId="status_info").child()[1].click()
                self.s20i_d.xpath('//*[@resource-id="div_dialog_cnt"]/android.view.View[2]').click()
                self.s20i_d(resourceId="dialog_btn_ok").click()
    ...
    def test01(self):
        ...
        with allure.step("第一步：拔出实体卡后重启设备"):
            while True:
                inp = input('\033[35;0m%s' % '请确认U3设备是否插入实体卡，如有插入实体卡请拔出实体卡, 确认操作完成后输入y继续(输入q退出测试)：')
                if inp == 'y':
                    break
                elif inp == 'q':
        ...
        with allure.step('第二步：测试设备关机，插入实体卡后开机，登陆webui界面查看是否显示选择登陆界面'):
            while True:
                inp = input('\033[35;0m%s' % '请将U3设备关机，并插入实体卡后开机，确认操作完成后输入y继续(输入q退出测试)：')
                if inp == 'y':
                    break
                elif inp == 'q':
                    log.error('用户退出测试，结束测试, inp:%s' % inp)
            ...
            if not test_result_01:
                self.ws1['G2'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test01_Step2_fail')
        ...
        with allure.step('第三步：选择实体卡登陆'):
            ...
            if all([test_result_02, test_result_03]):
                ...
                self.ws1['G2'] = "FAIL"
                log.error('测试结果test_result_01:%s,test_result_02：%s' % (test_result_01, test_result_02))
    ...
    def test02(self):
        ...
        with allure.step('第三步：辅助机连接测试机wifi，打开浏览器进入webUi界面，选择云卡登陆'):
            ...
            if not all([test_result_01, test_result_02]):
                self.ws1['G3'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='Step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
                allure.attach(file, '开机云卡登陆失败webui的截图', allure.attachment_type.PNG)
                assert False
        with allure.step("第四步: 切换SIM卡登陆"):
            self.s20i_uiOpration.login_webui()
            ...
            if not all([test_result_01, test_result_02]):
                self.ws1['G3'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='Step3_fail')
                file = open(FailScreenShot_file, 'rb').read()
        ...
        with allure.step("第五步：切换到云卡登陆"):
            ...
            if not test_result_03:
                self.ws1['G3'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='Step4_fail')
                file = open(FailScreenShot_file, 'rb').read()
                allure.attach(file, '切换云卡登陆失败webui的截图', allure.attachment_type.PNG)
            else:
                self.ws1['G3'] = "PASS"
    ...
    def test04(self):
        ...
        with allure.step("第二步：插入SIM卡，重启，选择云卡登陆"):
            ...
            if test_result_01:
                ...
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test04_step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test05(self):
        ...
        with allure.step("第二步：插入SIM卡，重启，选择云卡登陆"):
            ...
            test_result_01 = pytest.assume(self.u3.login_type() == 1)  # 判断测试机是否使用云卡
            if not test_result_01:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='Step5_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test06(self):
        ...
        with allure.step("第一步：选择云卡登陆，云卡登陆过程中拔插SIM卡"):
            ...
            test_result_01 = pytest.assume(self.s20i_d(resourceId="notice_cnt", text="You will use SIM 1(Manual) for Internet.").exists(10))  # 判断是否弹窗提示使用实体卡
            if not test_result_01:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='Step6_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test07(self):
        ...
        with allure.step("第二步：插入SIM卡，重启，选择云卡登陆"):
            ...
            if not all([test_result_01, test_result_02]):
                self.ws1['G8'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='Step8_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test08(self):
        ...
        with allure.step("第二步：插入带pin码的SIM卡，重启设备，登陆webui界面"):
            ...
            if not test_result_01:
                self.ws1['G9'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test08_step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
        ...
        with allure.step("第三步：切换SIM卡登陆"):
            ...
            if all([test_result_02, test_result_03]):
                ...
                self.ws1['G9'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test08_step3_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test09(self):
        ...
        with allure.step("第三步：拔掉SIM卡，登陆webui界面切换SIM卡登陆"):
            ...
            if not test_result_02:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test09_step3_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test11(self):
        ...
        with allure.step("第三步：插入另外一张SIM卡，登陆web界面切换SIM卡"):
            ...
            if not test_result_02:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test11_step3_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test13(self):
        ...
        with allure.step("第三步：插入另外一张SIM卡，登陆web界面"):
            ...
            if not test_result_02:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test013_step3_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test15(self):
        ...
        with allure.step("第二步：拔掉SIM卡，进入webui界面查看是否显示断开连接"):
            ...
            if not test_result_01:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test015_step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test17(self):
        ...
        with allure.step("第二步：拔掉SIM卡，进入webui界面查看是否显示断开连接"):
            ...
            test_result_01 = pytest.assume(self.s20i_d(text="Status: Absent").exists(5))    # 判断网络是否断开连接
            if not test_result_01:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test017_step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test18(self):
        ...
        with allure.step("第二步：拔掉SIM卡，进入webui界面查看是否显示断开连接"):
            ...
            test_result_01 = pytest.assume(self.s20i_d(text="Status: Absent").exists(5))    # 判断网络是否断开连接
            if not test_result_01:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test018_step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
        ...
        with allure.step("第四步：切换SIM卡登陆"):
            ...
            if not test_result_02:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test018_step4_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test19(self):
        ...
        with allure.step("第二步：拔掉SIM卡，进入webui界面查看是否显示断开连接"):
            ...
            test_result_01 = pytest.assume(self.s20i_d(text="Status: Disconnect").exists(5))    # 判断网络是否断开连接
            if not test_result_01:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test019_step2_fail')
                file = open(FailScreenShot_file, 'rb').read()
        ...
        with allure.step("第三步：插入另外一张SIM卡，查看设备网络是否还是断开连接"):
            ...
            if not test_result_02:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test19_step3_fail')
                file = open(FailScreenShot_file, 'rb').read()
        ...
        with allure.step("第四步：重启设备，查看webui界面是否显示选卡登陆界面"):
            ...
            if not test_result_03:
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test019_step4_fail')
                file = open(FailScreenShot_file, 'rb').read()
...
class Test_Flash:
    def setup_class(self):
        ...
        # 通过portal界面将U3设备恢复出厂设置，以防止U3设备内存不足无法push大文件
        self.s20i_uiOpration.connect_wifi(wifi_name=self.u3_ssid, password=self.u3_password)
        self.s20i_uiOpration.wipe_data()
        time.sleep(30)
        self.u3.wait_device_connect()
...
class Test_Display:
    ...
    def test01(self):
        ...
        test_result_01 = pytest.assume(card_info not in card_info_list)
        ...
        if all([test_result_01, test_result_02, test_result_03]):
            ...
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title="test01_CardStatus_fail")
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test02(self):
        ...
        test_result_04 = pytest.assume(ssid == self.u3_ssid)
        ...
        if all([test_result_01, test_result_02, test_result_03, test_result_04, test_result_05, test_result_06]):
            ...
            self.ws1['G9'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test02_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test03(self):
        ...
        test_result = pytest.assume(ssid == self.u3_ssid)
        if test_result:
            self.ws1['G10'] = "PASS"
        else:
            self.ws1['G10'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test03_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test04(self):
        ...
        test_result = pytest.assume(password == self.u3_password)
        if test_result:
            self.ws1['G11'] = "PASS"
        else:
            self.ws1['G11'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test04_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test05(self):
        ...
        if all([test_result_01, test_result_02]):
            ...
            self.ws1['G12'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test05_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test06(self):
        ...
        if client_num > 1:
            ...
            expire_connect_info = ' '.join([str(client_num), 'device is securely connected to Internet...'])
        test_result = pytest.assume(client_connect_info == expire_connect_info)
        if test_result:
            self.ws1['G13'] = "PASS"
        else:
            self.ws1['G13'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test06_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test07(self):
        ...
        if all(test_result_list):
            ...
            self.ws1['G14'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test07_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test08(self):
        ...
        if self.s20i_d(resourceId="div_wifi_black_lists").exists(3):
            ...
            if test_result:
                ...
                self.ws1['G15'] = "FAIL"
                FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test08_fail')
                file = open(FailScreenShot_file, 'rb').read()
    ...
    def test09(self):
        ...
        if all(test_result_list):
            ...
            self.ws1['G16'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test09_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test10(self):
        ...
        if all(test_result_list):
            ...
            self.ws1['G17'] = "FAIL"
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test10_fail')
            file = open(FailScreenShot_file, 'rb').read()
    ...
    def test11(self):
        ...
        if not all(test_result_list):
            FailScreenShot_file = self.s20i_uiOpration.screenShot(path=self.test_record_path, title='test11_fail')
            file = open(FailScreenShot_file, 'rb').read()
...
class Test_SystemStability:
    def setup_class(self):
        ...
        # 测试截图、视频的保存路径；如果不存在此路径，就创建此路径
        self.test_record_path = '/'.join([BASEDIR, 'TestRecord', project_name, "Test_SystemStability"])
        if not os.path.exists(self.test_record_path):
            os.makedirs(self.test_record_path)
        ...
        while True:
            ...
            self.s20i_uiOpration.open_webui()
            # 1、如果是选卡界面，选择云卡
            if self.s20i_d(resourceId="div_sim_select_notes").exists(10):
                self.s20i_d(resourceId="text_sim_0").click()
                self.s20i_d.click(0.501, 0.514)
                self.u3.wait_network_connect()
                self.s20i_uiOpration.open_webui()
                # 获取portal界面的显示语言，如果语言不是English，就将语言设置为English
                portal_language = self.s20i_d(resourceId="status_info").child()[1].get_text()
                ...
                self.s20i_d(resourceId="sim_select").click()
                # 判断实体卡是否为可选择状态
                sim_clickable = self.s20i_d(resourceId="sim_select_1").info["clickable"]
                if sim_clickable:
                    break
                else:
                    log.error("未检测到设备有实体卡, sim_clickable:%s" % sim_clickable)
                    time.sleep(0.5)
                    while True:
                        inp = input('\033[35;0m%s' % '请确认U3设备是否插入实体卡，如未插入实体卡请插入实体卡, 确认操作完成后输入y继续(输入q退出测试)：')
                        if inp == 'y':
                            break
                        elif inp == 'q':
                            log.error('用户退出测试，结束测试, inp:%s' % inp)
