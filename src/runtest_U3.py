...
from src.test_case.test_project_U3 import project_name
...
def main():
    ...
    # test_case_path = '/'.join([os.path.dirname(__file__), 'test_case', 'test_project_U3.py::Test_SystemStability::test09'])
    test_case_path = '/'.join([os.path.dirname(__file__), 'test_case', 'test_project_U3.py'])
    # --self_contained-html参数表示生成独立的html报告（即把CSS样式合并到html里）
    pytest.main(['-s', report, "--self-contained-html", test_case_path])

    # pytest.main(['-s', report, test_case_path])
